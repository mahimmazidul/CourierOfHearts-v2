...
export default function ComposePage({ onLetterCreated, onBack }: ComposePageProps) {
  ...
  const handleSend = useCallback(async () => {
    if (!content.trim()) { setError('Your letter has no words yet.'); return; }
    if (!recipient.trim()) { setError('Who is this letter for?'); return; }
    setError(''); setSending(true);
    try {
      const payload: CreateLetterPayload = {
        salutation, recipient: recipient.trim(), content: content.trim(),
  ...
  return (
    <div className="min-h-screen parchment-bg">
      <nav className="no-print sticky top-0 z-50 flex items-center justify-between px-4 py-3 md:px-8"
        ...
        <button onClick={() => setShowPreview(true)} className="font-heading text-[11px] tracking-[0.12em] text-ink/60 uppercase hover:text-ink transition-colors duration-500">Preview &rarr;</button>
      </nav>

      <div className="max-w-6xl mx-auto px-3 py-6 md:py-10 relative z-10">
        {/* Mobile tabs */}
        <div className="flex gap-1 mb-5 lg:hidden overflow-x-auto pb-1">
          {(['write','seal','decor','style'] as const).map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)}
              ...
              {tab === 'write' ? 'Write' : tab === 'seal' ? 'Seal' : tab === 'decor' ? 'Flowers' : 'Fonts'}
            </button>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 lg:gap-8">
            ...
            <div ref={paperRef} className="relative letter-paper rounded-sm"
              ...
              <div className="relative z-10 mb-6">
                <textarea value={content} onChange={(e) => setContent(e.target.value)}
                  ...
                  className="parchment-input w-full text-[17px] md:text-[18px] leading-[1.9em] min-h-[320px] md:min-h-[440px] py-2 resize-none ink-engraved"
          ...
          {/* ===== SIDEBAR — flat, no boxes ===== */}
          <div className="lg:col-span-1 relative z-10">
            <div className={`${activeTab !== 'seal' && activeTab !== 'write' ? 'hidden lg:block' : activeTab === 'write' ? 'hidden lg:block' : ''}`}>
            ...
            {/* Flowers */}
            <div className={`${activeTab !== 'decor' ? 'hidden lg:block' : ''} mb-5`}>
            ...
            {/* Fonts */}
            <div className={`${activeTab !== 'style' ? 'hidden lg:block' : ''} mb-5`}>
              <h3 className="font-heading text-[11px] tracking-[0.18em] text-ink/70 uppercase mb-3">Body Font</h3>
