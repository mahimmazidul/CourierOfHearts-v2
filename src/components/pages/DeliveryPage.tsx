...
function ReadingView({ letter, onBack }: { letter: Letter; onBack: () => void }) {
  ...
  useEffect(() => {
    ...
    const interval = window.setInterval(() => {
      const target = document.querySelector('.screen-only .rich-letter-content');
      if (!target) return;
  ...
  const flowersLayer = (opacity: number) => (letter.flowers || []).map((f) => {
  ...
  return (
    <div className="min-h-screen parchment-bg fade-slide-up">
      <nav className="no-print flex items-center justify-between px-4 py-3 md:px-8 relative z-20" style={{ borderBottom: '1px solid rgba(139,115,64,0.12)' }}>
        <button onClick={onBack} className="font-heading text-[11px] tracking-[0.12em] text-ink/70 uppercase hover:text-ink transition-colors duration-500">← Home</button>
            ...
            {printRenderedPages.map((pageHtml, pi) => (
              <article key={pi} className="print-letter relative letter-paper rounded-sm mb-8 last:mb-0"
                ...
                {pi === 0 && (
                  <div className="relative z-10">
                    {letter.letterDate && <p className="text-right font-display text-sm italic text-ink/45 mb-2 md:mb-3">{letter.letterDate}</p>}
                    {letter.customInitials && <div className="text-center mb-1 md:mb-2"><span className="font-uncial text-4xl md:text-5xl tracking-[0.12em] text-burgundy/35 select-none">{letter.customInitials}</span></div>}
                    {letter.crest !== 'none' && <div className="flex justify-center mb-2 md:mb-3"><CrestDecoration type={letter.crest} /></div>}
                    <div><OrnamentDivider className="w-24 md:w-32 mx-auto mb-2 md:mb-3" color="#8b7340" /></div>
                    {headerLine}
                  </div>
                )}

                <div className="rich-letter-content letter-flow relative z-10 ink-engraved" style={{ fontFamily: getFontFamilyByChoice(letter.bodyFont), fontSize: `clamp(${LETTER_UI.bodyFontSize}px, 1.2vw + 13px, ${LETTER_UI.bodyFontSizeMd}px)`, lineHeight: LETTER_UI.lineHeight, letterSpacing: '0.01em', wordSpacing: '0.04em', whiteSpace: 'pre-wrap' }} dangerouslySetInnerHTML={{ __html: pageHtml }} />

                {pi === printRenderedPages.length - 1 && (
                ...
                {printRenderedPages.length > 1 && <div className="absolute bottom-3 left-1/2 -translate-x-1/2 z-10"><span className="font-heading text-[9px] tracking-[0.2em] text-ink/30 uppercase">{pi + 1} of {printRenderedPages.length}</span></div>}
                {flowersLayer(LETTER_UI.readingFlowerOpacity)}
              </article>
            ))}
          </div>
